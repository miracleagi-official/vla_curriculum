# Ubuntu–Isaac Sim–Isaac Lab 기반 VLA/LeRobot 교육 커리큘럼 기획 보고서
...
본 보고서는 **Ubuntu + NVIDIA Isaac Sim + Isaac Lab + VLA(vision-language-action) 학습 도구**를 기반으로, **Hugging Face LeRobot**(데이터셋/로봇 제어/학습·평가 스택)과 연동되는 **실습 중심 교육 커리큘럼**을 제안한다. 핵심 설계 원칙은 “**하나의 표준 데이터/스키마로 시뮬레이션과 실로봇을 연결하는 것**”이다.

이를 위해 (1) 시뮬레이터에서 생성되는 센서·상태·행동·언어 지시를 **LeRobotDataset v3.0(Parquet+MP4, 메타데이터 기반 에피소드 재구성)**으로 기록·정규화하고, (2) 동일 포맷으로 실로봇에서 수집한 시연·실행 로그를 저장하며, (3) LeRobot의 VLA/IL/RL 정책 학습 및 벤치마크 평가 체인(예: LIBERO)을 통해 **심2리얼 전이 성능을 정량적으로 검증**하는 루프를 구성한다.

운영 관점의 주요 리스크는 **버전/런타임 불일치(드라이버·ROS 2)**와 **실로봇 안전·데이터 거버넌스**다. Isaac Sim 5.1 기준으로 Ubuntu 22.04/24.04와 특정 드라이버(예: Linux 580.65.06)가 요구된다.

---

## 기술 스택 전제조건 및 용어 정의

**대상 학습자:** 초급·중급·고급 수준별 커리큘럼을 별도 파일로 제공한다.
...
- 다만 VLA/정책 학습은 PyTorch CUDA 빌드에 의해 좌우되므로, 커리큘럼 운영 문서에는 **“학습용 PyTorch CUDA 빌드(예: cu12/cu13)는 선택 HW·드라이버에 맞춰 고정”**이라는 원칙을 포함해야 한다(세부 버전은 교육 기관 환경에 맞춰 별도 확정).  
...
### “VLA 학습 도구” 정의(본 커리큘럼에서의 의미)

요청에서 말하는 “VLA 학습 도구”는 특정 제품명이 명시되지 않았으므로, 본 보고서에서는 교육 운영에 사용할 도구를 다음과 같이 정의한다.
...
- **LeRobot을 “기본 VLA 학습 도구”로 채택**: LeRobot는 PyTorch 기반으로 IL/RL뿐 아니라 **VLA 모델 계열**을 구현·학습(예: SmolVLA 등)할 수 있다고 밝힌다. 또한 로봇 제어 인터페이스와 데이터셋 표준(LeRobotDataset)을 함께 제공하므로, 교육에서는 “수집→학습→배포”를 하나의 스택으로 연결할 수 있다.   
- **OpenVLA를 “확장(선택) VLA 학습 도구”로 병행**: OpenVLA는 “VLA 학습·파인튜닝을 위한 오픈소스 코드베이스”를 제공하며, LoRA 등 파라미터 효율적 미세조정과 평가 절차(시뮬 벤치마크 포함)를 문서화한다. 교육 과정의 고급 트랙에서는 비교 실험 대상으로 활용하기 좋다. 

---

## 커리큘럼 설계 원칙과 모듈 상세

### 설계 원칙(교육 대상이 아직 확정되지 않은 상황 반영)
...
4) **심2리얼은 “가설-변수-통계 검증”을 필수 산출물로 둔다**: 단순 데모에 그치지 않고, 도메인 랜덤화(시각·물리) 강도와 실데이터 보정량(예: few-shot 파인튜닝)에 따른 성능 변화를 통계적으로 검증한다.   

### 모듈 구성(권장 예시: 10모듈)

- 총 교육 기간: 운영 여건에 따라 조정  
- 아래 시간은 “권장 예시(총 60–90시간)”이며, 운영 상황에 따라 단축·확장할 수 있다(세부 시간의 합도 커스터마이징 가능).
...
- 이론: VLA에서 “언어-시각-행동 정렬”의 핵심, 데이터 믹스와 지시문(task) 설계의 역할. LeRobotDataset v3는 tasks.jsonl로 자연어 태스크를 정규화해 태스크 조건부 정책을 지원한다.   
- 실습 시나리오:  
...
- 산출물: (a) 학습된 체크포인트, (b) 평가 로그/성공률 리포트, (c) 에러 분석(실패 유형 분류 체계)

**모듈 G: 시뮬레이터–실로봇 연동(온라인 제어 + 오프라인 데이터)**  
- 이론: “오프라인 데이터 표준화”와 “온라인 제어 동기화”의 분리. ROS 2 브리지(토픽 송수신, `/clock`, `/tf`) 또는 ZMQ+Protobuf 브리지 대안을 비교한다.   
- 실습 시나리오:  
...
- 산출물: (a) 브리지 구동 체크리스트, (b) 지연시간/처리량 측정 로그, (c) 데이터↔제어 매핑 문서
...
- 실습 시나리오: E-stop 절차 리허설, 데이터 접근권한/보관정책 문서화, 사고/근접사고 보고 루프 구축. NIST는 로봇 운용 전 **비상정지 절차 숙지**를 강조한다.   
- 실습 시간: 4–6h  
...
| D | Isaac Lab 환경 생성 + DR 이벤트 훅 삽입 | 환경 코드, 학습 로그 |
| E | LeRobotDataset v3.0로 기록/로딩/검증 성공 | data/·videos/·meta/ 구조 |
| F | VLA 학습 1회 이상 + 벤치마크 평가 1회 이상 | 체크포인트, 평가 리포트 |
...
**시뮬→데이터 수집(권장 구현)**  
- Isaac Sim/Isaac Lab에서 관측값(obs: RGB/Depth/상태), 행동값(action: 조인트/EE), 타임스탬프를 프레임 단위로 수집  
...
- Isaac Sim은 ROS/ROS 2 브리지 확장을 통해 토픽·서비스 기반 통신을 제공하며, ROS 2 브리지는 확장에서 활성화한다(ROS와 ROS 2 브리지는 동시 활성화 불가).   
...
**좌표계 동기화(ROS 2 TF /tf)**  
- Isaac Sim은 TF 퍼블리시(카메라, 로봇 관절 체인 등)를 지원하며, `/tf` 토픽으로 트리 확인이 가능하다. 실로봇 측 좌표계(베이스/EE/카메라)와 시뮬 좌표계를 맞추기 위한 “프레임 계약(frame contract)”을 커리큘럼의 필수 산출물로 두는 것이 좋다.   

**센서/제어 주기 동기화**  
- 카메라 퍼블리시 주기와 관련 설정은 특정 노드/튜토리얼로 다뤄진다(교육에서는 “관측 주기≠제어 주기” 상황에서 지연/드롭이 정책 성능에 미치는 영향까지 확장 가능).   

### 시뮬레이터 vs 실제 비교표(필수 산출물)

| 항목 | 시뮬레이터(Isaac Sim/Isaac Lab) | 실로봇(LeRobot 지원 HW 예시) | 교육적 시사점 |
|---|---|---|---|
| 시간 기준 | `/clock` 기반 시뮬 타임을 퍼블리시 가능  | 시스템 타임/하드웨어 타임스탬프(플랫폼별로 다를 수 있음) | 동기화(지연·버퍼) 실습 필수 |
| 좌표계 | `/tf` 퍼블리시로 트리 구성 가능  | 로봇 URDF/캘리브레이션에 의존(플랫폼별 구현 차이) | “프레임 계약” 문서화가 성패 좌우 |
...
| 데이터 기록 | LeRobotDataset v3.0로 대규모 샤딩/스트리밍  | 동일 포맷으로 기록 가능(lerobot-record 등)  | “동일 포맷”이 심2리얼 실험 비용 절감 |
...
- 반복성(시드/환경 랜덤성):  
  - “시뮬 학습 시드”와 “실로봇 실행”을 분리해, 최소 3 시드×조건, 조건당 최소 20–50 트라이얼을 권장한다(현장 여건에 따라 조정)  
- 연속형 지표(시간/경로 길이):  
...
- **정지 기능의 계층을 명확히 구분**: 산업용 로봇 안전 표준 계열은 정상 정지/보호 정지/비상 정지와 같은 정지 기능의 구분을 전제로 한다(교육에서는 “E-stop은 마지막 수단이며, 보호 정지/소프트 리밋을 상시 적용”을 원칙으로 삼는다).   
...
- **실로봇 구역/시뮬 구역 분리**: 실로봇 구역은 접근 통제, 케이블 정리, 전원/배터리 관리, 작업 범위 표시를 표준화한다(세부 기준은 기관 환경에 맞춰 정한다).  
- **로그 기반 사고 예방**: 충돌/토크 제한 이벤트를 “실패”가 아니라 “안전 신호”로 분류해 자동 보고한다(근접사고 포함).  
- **원격/헤드리스 운용**: 컨테이너/스트리밍을 사용하는 경우에도, 운영자는 “정지/차단 루프(전원 차단 포함)”를 물리적으로 확보해야 한다.   

### 데이터 관리·윤리(로봇 데이터셋의 특수성 반영)

- LeRobotDataset v3.0는 대규모 데이터(센서 모달리티+멀티카메라)를 메타데이터 기반으로 관리하고, 허브 스트리밍까지 지원한다. 교육에서는 **데이터 무결성(finalize 필수), 메타데이터(스키마·통계·태스크) 관리**를 운영 규칙으로 고정한다.   
- 데이터 거버넌스는 AI 준비도의 핵심 역량으로, 정책·보안·투명성을 데이터 라이프사이클 전반에 적용해야 한다(공공 데이터의 AI 준비 가이드에서도 거버넌스 중요성을 강조).   
- 실데이터(카메라 영상)가 사람/주거환경을 포함할 수 있으므로, 수집 전 동의/비식별화/보관기간/접근권한(역할 기반)을 명시해야 한다(세부 법규 적용 범위는 기관·국가 정책에 따라 별도 검토).

---

## 평가·피드백 체계와 예산·일정

### 평가·피드백 방법 (정량/정성 및 루브릭)

#### 1. 정량 지표(권장)

- **환경 구축 재현성 점수**: 버전표(OS/드라이버/Isaac Sim/Isaac Lab/LeRobot)와 실행 로그 제출(동일 결과 재현 여부)  
- **데이터셋 품질 점수**: (a) 스키마 일관성(meta/info.json), (b) stats.json 정상 생성, (c) tasks.jsonl 태스크 커버리지, (d) 프레임 드롭 등 품질 지표(영상 품질 포함)   
- **정책 성능**: 성공률/시간/안전 이벤트(실로봇 운용 환경이 없는 경우 해당 항목은 별도 평가 항목으로 분리)  
...
**대체 옵션**  
- “GPU 부족” 시: 시뮬 장면 단순화/센서 수 축소, 학습은 클라우드/공유 서버로 이전(운영 정책에 따라 결정)  
...
### 비용 추정(기준 시점: 2026-03-16, 통화·환율 별도)
...
**예산 템플릿(예시, USD 기준)**  
- 컴퓨트 워크스테이션 1대: (RTX 4080급 GPU + 기타 부품) → **약 $2,500–$5,000+ (구성/지역에 따라 변동, 환율 별도)**   
- 저가 실로봇팔 1세트: **$220–$270+ (패키지/프린트 포함 여부에 따라 변동)**   
- 필수 소모품/안전: 작업 장갑/보안경/공구/케이블/전원장치 등(기관 환경에 따라 별도 산정)  
...
timeline
  ...
  Week 5  : 모듈 E(LeRobot/LeRobotDataset v3.0)
...
flowchart LR
  ...
  subgraph ONLINE["Online Bridge (선택)"]
    O1["ROS 2 Bridge: /clock /tf images/actions"]
    O2["ZMQ+Protobuf Bridge (선택)"]
  end
...
이 플로우는 (1) **데이터는 LeRobotDataset로 통합**하고, (2) **온라인 제어는 ROS 2 또는 ZMQ로 분리**하며, (3) **도메인 랜덤화는 학습 조건(실험 변수)으로 승격**하는 구조를 따른다. 
